//collinsion new live - (File size updation)
jQuery(document).ready(function ($) {
  ...
  $("#next").click(function () {
    ...
    $("#section-1-button, #section-2-button, #section-3-button").removeAttr(
      "data-bs-toggle",
    );
  ...
  $(".save_draft_5").click(function () {
    ...
    if (elem.files.length) {
      ...
      // NEW: Combined size limit changed to 20 MB
      if (!individualFileSizeExceeded && sum <= 20971520) {
        let ele = ["api_call_failed_filelist"];
        clearError(ele);
        ...
        if (individualFileSizeExceeded) {
          ...
          addErrorMessage(
            "api_call_failed_filelist",
            "Total file size of all attachments cannot exceed 20 MB",
          );
        }
      }
    } else {
  ...
  function fileUploadCallback() {
    ...
    if (elem.files.length) {
      ...
      // NEW: Combined file size should not exceed 20 MB
      if (!individualFileSizeExceeded && sum <= 20971520) {
        let ele = ["api_call_failed_filelist"];
        clearError(ele);
