jQuery(document).ready(function ($) {
  ...
  function buildPolicyUI(policyData) {
    ...
    $("#collapseSection2 .list-policy-names").append(
  ...
  $("#save_and_continue2").click(function () {
    ...
    if (len <= 0) {
      addErrorMessage("check-finder", "Please check at least one");
    } else {
  ...
  function createClaimRequest(claimObject, fieldId) {
    ...
    fetch("https://claim-sandbox.collinsonnis.com/api/claim", requestOptions)
      ...
      .then(function (result) {
        ...
        } else if (result == "Api error while getting the changes") {
          console.log("unauthorized token please try again sometime");
        } else {
  ...
  $("#save_and_continue6").click(function () {
    ...
    if (checkedfilled(payment_list) == true) {
      ...
      console.log(accountNumber, iBanNumber, sortCode, countryCode[0].akey);
      //method one params
      //iBan
      //CountryCode
      //method two params
  ...
  $(".new-ticket-submit-button").click(function () {
    ...
    let MainClientId = $("#0").attr("data-clientId");
    console.log("Main ");
    let body = {
      internalpolicyNumber: policyNumber,
      ClaimNumber: ClaimInitiatedNumber,
      FromDate: "2020-03-13T09:37:38.447Z",
      ToDate: "2020-03-13T09:37:38.447Z",
      UseContactInformationFromLeadInsured: true,
      MainContactClientId: MainClientId,
    };
    //otherclients senarios logic
    let clients = $("input[name='insured_1']:checked").length;
    if (clients > 1) {
      console.log("more than one");
      let temp = [];
      for (let i = 1; i < $("input[name='insured_1']:checked").length; i++) {
        let clientIds = $("#" + i).attr("data-clientId");
        temp.push(clientIds);
      }
      body["OtherInsuredClientIds"] = temp;
      console.log("If statement --->", body);
    } else {
      body["OtherInsuredClientIds"] = [];
      console.log("else statement --->", body);
    }
    ...
    if (bankResult) {
      ...
      bankDetails["BankName"] = bankResult.bankName;
      bankDetails["IBan"] = bankResult.iBan;
      bankDetails["BankAddressLine1"] = bankResult.bankAddressLine1;
      bankDetails["BankAddressLine2"] = bankResult.bankAddressLine2;
      bankDetails["BankAddressLine3"] = bankResult.bankAddressLine3;
      bankDetails["PostCode"] = bankResult.postCode;
      bankDetails["City"] = bankResult.city;
      bankDetails["Swift"] = bankResult.swift;
      bankDetails["AccountNumber"] = bankResult.accountNumber;
      bankDetails["SortCode"] = bankResult.sortCode;
      bankDetails["CountryCode"] = bankResult.countryCode;
      body["BankAccountDetails"] = bankDetails;
      console.log("formed body -->", body);
    } else {
      Console.log("Please validate bank details");
      showError = "Please validate bank details";
    }
    ...
    if (reasonClaim[0].akey == "01_02_Canx") {
      ...
      let ItemTypeDamaged = getAKey(
        "helpdesk_ticket_custom_field_cf_item_type_2321673",
        ItemTypeDamaged
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          ItemTypeDamaged[0].qkey,
          ItemTypeDamaged[0].akey,
          fieldTypeText
        )
      );
      ...
      let DamageItemOwner = getAKey(
        "helpdesk_ticket_custom_field_cf_who_owns_the_item_2321673",
        DamageItemOwner
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          DamageItemOwner[0].qkey,
          DamageItemOwner[0].akey,
          fieldTypeText
        )
      );
      ...
      let LossOrTheft = getAKey(
        "helpdesk_ticket_custom_field_cf_are_you_claiming_for_a_loss_or_theft_2321673",
        LossOrTheft
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(LossOrTheft[0].qkey, LossOrTheft[0].akey, fieldTypeText)
      );
      let LossTheftWhere = getAKey(
        "helpdesk_ticket_custom_field_cf_where_did_the_loss_or_theft_take_place_2321673",
        LossTheftWhere
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          LossTheftWhere[0].qkey,
          LossTheftWhere[0].akey,
          fieldTypeText
        )
      );
      ...
      let ItemOwner = getAKey(
        "helpdesk_ticket_custom_field_cf_who_owns_the_item86910_2321673",
        ItemOwner
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(ItemOwner[0].qkey, ItemOwner[0].akey, fieldTypeText)
      );
      let HomeInsurance = getAKey(
        "helpdesk_ticket_custom_field_cf_do_you_have_home_contents_insurance_2321673",
        yesNo
      ...
      let NoReportReason = getAKey(
        "helpdesk_ticket_custom_field_cf_please_confirm_why_you_did_not_report_the_loss_or_theft6975_2321673",
        NoReportReason
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          NoReportReason[0].qkey,
          NoReportReason[0].akey,
          fieldTypeText
        )
      );
      let LossTheftVehicle = getAKey(
        "helpdesk_ticket_custom_field_cf_whereabouts_in_the_vehicle_was_the_items_when_stolen_2321673",
        LossTheftVehicle
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          LossTheftVehicle[0].qkey,
          LossTheftVehicle[0].akey,
          fieldTypeText
        )
      );
      ...
      let CancellationPerson = getAKey(
        "helpdesk_ticket_custom_field_cf_who_has_caused_the_cancellation_2321673",
        CancellationPerson
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          CancellationPerson[0].qkey,
          CancellationPerson[0].akey,
          fieldTypeText
        )
      );

      let ReasonForCancellation = getAKey(
        "helpdesk_ticket_custom_field_cf_are_you_currently_still_abroad_2321673",
        ReasonForCancellation
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          ReasonForCancellation[0].qkey,
          ReasonForCancellation[0].akey,
          fieldTypeText
        )
      );
      ...
      let WhoAdvisedToCancel = getAKey(
        "helpdesk_ticket_custom_field_cf_who_advised_to_cancel_2321673",
        WhoAdvisedToCancel
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          WhoAdvisedToCancel[0].qkey,
          WhoAdvisedToCancel[0].akey,
          fieldTypeText
        )
      );
      let CancellationTypeOfTrip = getAKey(
        "helpdesk_ticket_custom_field_cf_what_type_of_trip_did_you_book137913_2321673",
        CancellationTypeOfTrip
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          CancellationTypeOfTrip[0].qkey,
          CancellationTypeOfTrip[0].akey,
          fieldTypeText
        )
      );
      let CancellationIllnessCondition = getAKey(
        "helpdesk_ticket_custom_field_cf_what_medical_condition_has_resulted_in_cancellation748907_2321673",
        CancellationIllnessCondition
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          CancellationIllnessCondition[0].qkey,
          CancellationIllnessCondition[0].akey,
          fieldTypeText
        )
      );
      let TripTransportType = getAKey(
        "helpdesk_ticket_custom_field_cf_what_type_of_transport_was_included_in_your_booking146711_2321673",
        TripTransportType
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          TripTransportType[0].qkey,
          TripTransportType[0].akey,
          fieldTypeText
        )
      );
      let currencyType = getAKey(
      ...
      let CURTAILMENTREASON = getAKey(
        "helpdesk_ticket_custom_field_cf_why_did_you_shorten_your_trip_2321673",
        CURTAILMENTREASON
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          CURTAILMENTREASON[0].qkey,
          CURTAILMENTREASON[0].akey,
          fieldTypeText
        )
      );
      ...
      let WhoDentalExpenses = getAKey(
        "helpdesk_ticket_custom_field_cf_who_incurred_dental_expenses930485_2321673",
        WhoDentalExpenses
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          WhoDentalExpenses[0].qkey,
          WhoDentalExpenses[0].akey,
          fieldTypeText
        )
      );
      let DentalTreatment = getAKey(
        "helpdesk_ticket_custom_field_cf_what_type_of_dental_treatment_did_you_need_2321673",
        DentalTreatment
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          DentalTreatment[0].qkey,
          DentalTreatment[0].akey,
          fieldTypeText
        )
      );
      let currencyType = getAKey(
      ...
      let TransportationMethod = getAKey(
        "helpdesk_ticket_custom_field_cf_what_transport_was_delayed_2321673",
        TransportationMethod
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          TransportationMethod[0].qkey,
          TransportationMethod[0].akey,
          fieldTypeText
        )
      );
      let ReasonForDelay = getAKey(
        "helpdesk_ticket_custom_field_cf_what_was_the_reason_for_the_delay_2321673",
        ReasonForDelay
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          ReasonForDelay[0].qkey,
          ReasonForDelay[0].akey,
          fieldTypeText
        )
      );
      let WhichJourneyNew = getAKey(
        "helpdesk_ticket_custom_field_cf_on_which_part_of_the_journey_were_you_delayed_2321673",
        WhichJourneyNew
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          WhichJourneyNew[0].qkey,
          WhichJourneyNew[0].akey,
          fieldTypeText
        )
      );
      ...
      let WhoMedicalExpenses = getAKey(
        "helpdesk_ticket_custom_field_cf_who_incurred_medical_expenses_2321673",
        WhoMedicalExpenses
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          WhoMedicalExpenses[0].qkey,
          WhoMedicalExpenses[0].akey,
          fieldTypeText
        )
      );
      let MedicalInjuryIllness = getAKey(
        "helpdesk_ticket_custom_field_cf_are_you_claiming_for_an_injury_or_illness644442_2321673",
        MedicalInjuryIllness
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          MedicalInjuryIllness[0].qkey,
          MedicalInjuryIllness[0].akey,
          fieldTypeText
        )
      );
      let currencyType = getAKey(
      ...
      let MissedDepartureReason = getAKey(
        "helpdesk_ticket_custom_field_cf_what_caused_you_to_miss_your_departure_2321673",
        MissedDepartureReason
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          MissedDepartureReason[0].qkey,
          MissedDepartureReason[0].akey,
          fieldTypeText
        )
      );
      let MissedDepDelayReason = getAKey(
        "helpdesk_ticket_custom_field_cf_please_confirm_the_reason_for_delay_2321673",
        MissedDepDelayReason
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          MissedDepDelayReason[0].qkey,
          MissedDepDelayReason[0].akey,
          fieldTypeText
        )
      );
      ...
      let MissedDepWhichJourney = getAKey(
        "helpdesk_ticket_custom_field_cf_on_which_part_of_the_journey_did_you_miss_your_departure_2321673",
        MissedDepWhichJourney
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          MissedDepWhichJourney[0].qkey,
          MissedDepWhichJourney[0].akey,
          fieldTypeText
        )
      );
      ...
      let MissedDepExpenseType = getAKey(
        "helpdesk_ticket_custom_field_cf_what_type_of_additional_expenses_are_you_claiming_for_2321673",
        MissedDepExpenseType
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          MissedDepExpenseType[0].qkey,
          MissedDepExpenseType[0].akey,
          fieldTypeText
        )
      );
      let currencyType = getAKey(
      ...
      let SubAccomReason = getAKey(
        "helpdesk_ticket_custom_field_cf_what_caused_you_to_leave_your_original_accommodation_2321673",
        SubAccomReason
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          SubAccomReason[0].qkey,
          SubAccomReason[0].akey,
          fieldTypeText
        )
      );
      let SubAccomTripType = getAKey(
        "helpdesk_ticket_custom_field_cf_what_type_of_trip_did_you_book_2321673",
        SubAccomTripType
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          SubAccomTripType[0].qkey,
          SubAccomTripType[0].akey,
          fieldTypeText
        )
      );
      let currencyType = getAKey(
      ...
      let SkiPackClaimReason = getAKey(
        "helpdesk_ticket_custom_field_cf_which_winter_sports_item_are_you_claiming_for_2321673",
        SkiPackClaimReason
      );
      ClaimQaAndAnswers.push(
        submitClaimBody(
          SkiPackClaimReason[0].qkey,
          SkiPackClaimReason[0].akey,
          fieldTypeText
        )
      );
    ...
    if (showError) {
      console.log("Need to stop submit Api and need to show error message");
    } else {
      createSubmitClaim(body, ".new-ticket-submit-button");
    }
  });
  ...
  function createSubmitClaim(body, fieldId) {
    ...
    fetch(
      "https://claim-sandbox.collinsonnis.com/v2/claim/submission",
      requestOptions
    )
      .then((response) => response.text())
      .then(function (result) {
