jQuery(document).ready(function ($) {
  ...
  function getPolicyDetails(policyNumber, dateOfBirth, fieldId) {
    ...
    fetch(
      ...
      .then(function (result) {
        ...
        console.log("policy --->", JSON.parse(result).status);
        let status = JSON.parse(result).status;
        if (succesStatus.includes(status)) {
          console.log("entered in sucess policy");
  ...
  $("#save_and_continue2").click(function () {
    ...
    if (len <= 0) {
      addErrorMessage("check-finder", "Please Select at least one ClientID");
    } else {
  ...
  function createClaimRequest(claimObject, fieldId) {
    ...
    fetch("https://claim-sandbox.collinsonnis.com/api/claim", requestOptions)
      ...
      .then(function (result) {
        ...
        console.log("claim status --->", JSON.parse(result).status);
        let status = JSON.parse(result).status;
        if (succesStatus.includes(status)) {
          console.log("inside sucess claim ", JSON.parse(result));
  ...
  function getUploadFiles(files, fieldId) {
    ...
    fetch(
      ...
      .then(function (result) {
        ...
        console.log("upload file status --->", JSON.parse(result).status);
        let status = JSON.parse(result).status;
        if (succesStatus.includes(status)) {
  ...
  function ValidateBankMethodOne(iBanNumber, countryCode, fieldId) {
    ...
    fetch(
      ...
      .then(function (result) {
        ...
        console.log(
          " detils validate method  one status --->",
          JSON.parse(result).status
        );
        let status = JSON.parse(result).status;
        if (succesStatus.includes(status)) {
          bankResult = result;
          console.log("inside sucess-->", bankResult);
        }
        if (status == 401) {
          getJWTToken(fieldId);
        }
  ...
  function ValidateBankMethodTwo(
    ...
    fetch(
      ...
      .then(function (result) {
        ...
        console.log(
          " detils validate method  two status --->",
          JSON.parse(result).status
        );
        let status = JSON.parse(result).status;
        if (succesStatus.includes(status)) {
          console.log("sucess bank details two--->", result);
        }
        if (status == 401) {
          getJWTToken(fieldId);
        }
  ...
  function createSubmitClaim(body, fieldId) {
    ...
    fetch(
      ...
      .then(function (result) {
        ...
        console.log(" submission status --->", JSON.parse(result).status);
        let status = JSON.parse(result).status;
        if (succesStatus.includes(status)) {
          console.log("sucess submission --->", result);
          $("button .new-ticket-submit-button").trigger("click");
        }
        if (status == 401) {
          getJWTToken(fieldId);
        }
